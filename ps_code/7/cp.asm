;;; ============================================================================
;;; cp.asm -- a simple version of the unix `cp` program, using system calls
;;; Programmer: Mayer Goldberg, 2026        (study annotations added)
;;;
;;; Usage: cp source-file destination-file
;;;
;;; *** THIS FILE IS BYTE-FOR-BYTE code-0022.asm FROM "lectures code ". ***
;;;   Confirm it yourself:
;;;       diff <(sed -n '/SIZE equ/,$p' originals/ps_code/7/cp.asm) \
;;;            <(sed -n '/SIZE equ/,$p' "originals/lectures code/code-0022.asm")
;;;   The only difference in the whole file is one leading tab. So the notes
;;;   below are the same notes, with the paths adjusted -- and the two are worth
;;;   keeping in step if you edit either.
;;;
...
;;;   system call here is followed by `cmp rax, 0` / `jl <some error label>`.
;;;
...
;;;   is fast. One system call per 1024 bytes rather than one per byte. Change
...
;;;   so use filenames RELATIVE TO ps_code/7, not absolute paths.
;;;
;;;   printf 'hello world\nsecond line\n' > ps_code/7/sample.txt
;;;   ./asm "ps_code/7/cp.asm" sample.txt copy.txt
;;;   cat ps_code/7/copy.txt
;;;   cmp ps_code/7/sample.txt ps_code/7/copy.txt && echo IDENTICAL
;;;
;;;   Try the error paths:
;;;   ./asm "ps_code/7/cp.asm"                      # usage
;;;   ./asm "ps_code/7/cp.asm" nosuchfile out.txt   # cannot open
;;;
;;;   Copy something bigger than one block and check the byte count:
;;;   ./asm "ps_code/7/cp.asm" cp.asm cp_copy.asm
;;;   ls -l ps_code/7/cp.asm ps_code/7/cp_copy.asm
;;;
;;;   Then clean up:  rm -f ps_code/7/{sample,copy}.txt ps_code/7/cp_copy.asm
;;;
;;; DEBUG IT
;;;   printf 'hello\n' > ps_code/7/sample.txt
;;;   ./debug "ps_code/7/cp.asm" sample.txt copy.txt
;;;
;;;   Useful session:
;;;     break cp.asm:NN            put NN on the first `syscall` (the open)
;;;     c
...
;;;
;;;   Watch a block move:
;;;     break cp.asm:NN            NN on the `syscall` inside .loop (the read)
;;;     c
;;;     si
...
;;;     ./debug "ps_code/7/cp.asm" nosuchfile out.txt
;;;     break cp.asm:NN            NN on the first syscall
;;;     c
;;;     si
...
;;;   walk. The return address is kept in rcx and the flags in r11 by the
;;;   hardware -- print them after any `syscall` here and you are looking at the
;;;   return mechanism directly. This is why rcx is never used to pass a syscall
;;;   argument.
;;;
...
;;;   `.print_and_exit` tail. One exit, five entries -- the same many-in/one-out
;;;   pattern as code-0005.asm's answer labels, and the same reason: the common
;;;   part is written once. Notice the last one has no `jmp`, because
;;;   `.print_and_exit` is the next line and control simply falls into it.
;;;
